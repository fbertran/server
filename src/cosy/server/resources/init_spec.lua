...
describe ("cosyverif api", function ()
  ...
  describe ("route '/resources'", function ()
    ...
    it ("answers to HEAD", function ()
      local status = request (app, "/resources", {
        method = "HEAD",
      })
  ...
  describe ("route '/resources/:resource'", function ()
    ...
    it ("answers to HEAD for a non-existing resource", function ()
      ...
      local status = request (app, "/resources/" .. resources.rahan, {
        method  = "DELETE",
        headers = { Authorization = "Bearer " .. token},
      })
      assert.are.same (status, 204)
      status = request (app, "/resources/" .. resources.rahan, {
        method = "HEAD",
      })
    ...
    it ("answers to PUT for another user with Authorization", function ()
      local token  = Test.make_token (Test.identities.rahan)
      local status = request (app, "/resources/" .. resources.crao, {
        method  = "PUT",
        headers = { Authorization = "Bearer " .. token},
      })
    ...
    it ("updates resource on PUT with Authorization", function ()
      ...
      assert.are.same (status, 204)
      local result
      status, result = request (app, "/resources/" .. resources.rahan, {
        method = "GET",
      })
      assert.are.same (status, 200)
      result = Util.from_json (result)
    ...
    it ("Updates resources on PATCH with Authorization", function ()
      local token  = Test.make_token (Test.identities.rahan)
      local status = request (app, "/resources/" .. resources.rahan, {
        method  = "PATCH",
        headers = {
    ...
    it ("allows edition", function ()
      local token  = Test.make_token (Test.identities.rahan)
      local status, result = request (app, "/resources/" .. resources.rahan, {
        method  = "GET",
        headers = { Authorization = "Bearer " .. token},
      })
      assert.are.same (status, 200)
    ...
    for _, method in ipairs { "DELETE", "GET", "HEAD", "OPTIONS", "PATCH", "PUT", "POST" } do
      it ("correcly fails for invalid argument to " .. method, function ()
        local token  = Test.make_token (Test.identities.rahan)
        local status = request (app, "/resources/myresource", {
          method = method,
          headers = { Authorization = "Bearer " .. token},
        })
